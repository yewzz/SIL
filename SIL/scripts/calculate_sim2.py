    ...
    for idx, data in enumerate(ref_caps):
        ...
        with torch.no_grad():
            ...
            for i in range(len(props)):
                if props[i, 0] < props[i, 1]:
                    z = images_features[4 * props[i, 0]:4 * props[i, 1]]
                    z = z.mean(0)
                    z = z / z.norm(dim=-1, keepdim=True)
                    map_features.append(z.mean(0))
                else:
                    map_features.append(images_features[4 * props[i, 0]].squeeze(0))
            ...
            pos = torch.argsort(similarity, descending=True)[0,:32]
            top_sim = x[0, pos]
            x[x < 0.7] = 0
